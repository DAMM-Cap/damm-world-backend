...
def get_lagoon_deployments(chain_id: int, index: int):    
    if chain_id not in deployments:
        raise ValueError(f"Chain ID {chain_id} not supported")
    deploymentsArray = deployments[chain_id]
    if index < len(deploymentsArray):
        return deploymentsArray[index]
    else:
        raise ValueError(f"Index {index} out of range for chain ID {chain_id}")
    
def get_lagoon_deployments_by_vault_address(chain_id: int, vault_address: str):
    if chain_id not in deployments:
        raise ValueError(f"Chain ID {chain_id} not supported")
    deploymentsArray = deployments[chain_id]
...
def get_count_lagoon_deployments_by_chain_id(chain_id: int):
    if chain_id not in deployments:
        raise ValueError(f"Chain ID {chain_id} not supported")
    return len(deployments[chain_id])